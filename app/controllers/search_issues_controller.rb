class SearchIssuesController < ApplicationController
  ...
  def index
    ...
    log "Did you mean settings: #{Setting.plugin_redmine_didyoumean.to_json}"
    ...
    if !@tokens.empty?
      ...
      case Setting.plugin_redmine_didyoumean['project_filter']
        ...
        when '0'
          ...
          log "Unrecognized option for project filter: [#{Setting.plugin_redmine_didyoumean['project_filter']}], skipping"
      end

      if project_tree
        # check permissions
        scope = project_tree.select { |p| User.current.allowed_to?(:view_issues, p) }.map(&:id)
        log "Set project filter to #{scope}"
        conditions += ' AND project_id in (?)'
        variables << scope
      end

      if Setting.plugin_redmine_didyoumean['show_only_open'] == '1'
        valid_statuses = IssueStatus.where('is_closed <> ?', true).pluck(:id)
        log "Valid status ids are #{valid_statuses}"
        conditions += ' AND status_id in (?)'
        variables << valid_statuses
      end

      if !issue_id.nil?
        log "Excluding issue #{issue_id}"
      ...
      log "#{@count} results found, returning the first #{@issues.length}"
  ...
  def log message
    logger.debug "redmine_didyoumean: #{ message }"
  end
end
